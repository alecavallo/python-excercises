"""
Simple unit tests for Meeting Scheduler API using complete mocking
"""
...
class TestSimpleUnitTests:
    """Simple unit tests with complete mocking"""
    ...
    def test_very_large_page_size_at_limit(self, mock_get_available_slots, client):
    ...
    def test_book_endpoint_with_params(self, client):
        """Test book endpoint with parameters"""
        response = client.post("/availability/book?slot_id=1&email=test@example.com")

        # Should return either 200 (success), 404 (not found), 409 (already booked), or 500 (server error)
